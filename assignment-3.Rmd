...
for (i in 1:9999) {
          ...
          coef_permuted <- coef(lm(permuted_rating ~ 
                                   language, 
                                   data = permuted_MOP))[2]
          coeficients_H1[i] <- coef_permuted
}
...
for (i in 1:9999) {
      ...
      fake_good <- dplyr::data_frame(fake_rating = rnorm(n = (dim(good)[1]/2), 
                              mean = mn1, sd = sd1),
                              fake_language = "good")
                      
      fake_bad <- dplyr::data_frame(fake_rating = rnorm(n = (dim(bad)[1]/2), 
                            mean = mn2, sd = sd2),
                            fake_language = "bad")
      ...
      coef_permuted <- coef(lm(fake_rating ~ 
                               fake_language, 
                               data = H2_data))[2]
      coeficients_H2[i] <- coef_permuted
}
...
coefs_table <- dplyr::data_frame(H1 = coeficients_H1,
                                 H2 = coeficients_H2)
...
for (i in 1:9999) {
      ...
      H3_data_obs <- dplyr::bind_rows(fake_good,
                                  fake_bad)
      
      H3_data_obs_fake <- H3_data_obs %>%
                            dplyr::mutate(
      ...
      coef_permuted <- coef(lm(similar_rating_to_observed ~ 
                               fake_language, 
                               data = H3_data_obs_fake))[2]
      coeficients_H3[i] <- coef_permuted
}
```
...
One possible distribution of newly obtained data is as follows:

```{r include = FALSE}
simulated_H3_plot <- ggplot2::ggplot(H3_data_obs_fake, 
                                 ggplot2::aes(x=similar_rating_to_observed, 
                                              fill=fake_language)) +
                       ggplot2::geom_histogram(
                          alpha=0.4, position="identity")
```
...
There is now many values at 0 or at 100. This is similar to the observed distributions.
...
coefs_table2 <- dplyr::data_frame(H1 = coeficients_H1,
                                 H3 = coeficients_H3)
coefs_joined2 <- tidyr::gather(coefs_table2,
                                 `H1`,
                                 `H3`,
                                 key = "Hypothesis",
                                 value = "Coef")

plot_coefs_H1H3 <- ggplot2::ggplot(coefs_joined2, 
                                 ggplot2::aes(x=Coef, 
                                              fill=Hypothesis)) +
                            ggplot2::geom_histogram(
                              colour = "black", position="identity",
