...
permute_for_h2 <- function(observed_mean_diff,
  ...
  for (i in 1:9999) {
    ...
    if (adjust_values >= 1) {
      H2_data <- H2_data %>%
        dplyr::mutate(similar_rating_to_observed =
                        ifelse(
                          fake_rating < 0,
                          0,
...
plot_permuted_data <- function(coefficients_H1,
                               coefficients_H2,
                               transparency,
                               vector_size) {
  ...
  plot_coefs_H1H2 <- ggplot2::ggplot(coefs_joined,
    ...
    ggplot2::geom_histogram(
      ...
      bins = 80,
      alpha = transparency) +
    ggplot2::coord_cartesian(xlim = vector_size)
